...
void CA::displayGeneration(int gen)
{
    ...
    for (int i = 0; i < genCells.size(); i++)
    {
        if (genCells[i] == 1)
        {
            float myColor1=ofMap(i,0,genCells.size(),0,255);
            float myColor2=ofMap(gen,0,generations.size(),0,255);
            ofSetColor(myColor1,0,myColor2,255);
        }
        else
        {
        ...
        ofDrawRectangle(i*cellWidth, gen*cellWidth, cellWidth, cellWidth);
    }
}
